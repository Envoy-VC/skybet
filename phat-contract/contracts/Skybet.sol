...
contract Skybet is PhatRollupAnchor, Ownable {
    ...
    function addStake(uint256 _id, Outcome _stakeType, uint256 _amount) public {
        ...
        require(STAKE_TOKEN.allowance(msg.sender, address(this)) >= _amount);
        require(STAKE_TOKEN.transferFrom(msg.sender, address(this), _amount));
    ...
    function withrawWinnings(uint256 _id) public {
        require(_id <= gameCount, "Game does not exist");
        ...
        require(bet.totalTokensStaked > 0, "You have not staked any tokens");
        ...
        require(
            STAKE_TOKEN.transferFrom(address(this), msg.sender, rewardTokens)
        );
    ...
    function declareResult(uint256 _id) public {
        ...
        _pushMessage(
            abi.encode(id, game.startAt, game.endAt, game.token.symbol)
        );
        nextRequest += 1;
    }
    ...
    function _onMessageReceived(bytes calldata action) internal override {
        ...
        (uint respType, uint id, uint256 valueAtStart, uint256 valueAtEnd) = abi
            .decode(action, (uint, uint, uint256, uint256));
    ...
    function tokenExists(
        ...
        for (uint i = 0; i < tokenCount; i++) {
            if (
                keccak256(bytes(_symbol)) == keccak256(bytes(Tokens[i].symbol))
            ) {
                return true;
            }
        }
